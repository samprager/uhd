function varargout = freqstack2(varargin)
% freqstack - Synthesize wide band chirp signal with frequency stacking
%
% Usage:
%    [data_u, (x_tx)] = freqstack2(trials,fs)
%    [data_u, (x_tx)] = freqstack2(trials,fs,dfc,Bs,upfac)
%    [data_u, (filt_u), (x_tx)] = freqstack2(trials,filt,fs,dfc,Bs,upfac)
% Inputs:
...
%         plot(real(zn(i,:))); title('zn');
    end
    
%     t_down = linspace(-size(data_in,2)/(2*fs),size(data_in,2)/(2*fs),size(data_in,2));
%     figure; hold on; 
%     for i=1:N
%         zn_temp = conv(data_in(i,:).*exp(1i*2*pi*dfn(i)*t_down),gn(i,:));
%         plot(real(zn_temp)); title('zn temp');
%     end
...
elseif(nargin==6)
    ...
    for i=1:N
        zn = [zn;conv(data(i,:).*exp(1i*2*pi*dfn(i)*t),gn(i,:))];
        xn = [xn;conv(filt.*exp(1i*2*pi*dfn(i)*t),gn(i,:))];
    end
